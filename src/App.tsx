...
function parseProjectPanoramaMarkdown(content: string): GraphData | null {
  const jsonBlockMatch =
    content.match(
      /##\s*4\.\s*函数调用链（JSON）[\s\S]*?```json\s*([\s\S]*?)```/i,
  ...
  const repoName =
    content.match(/- 项目名称:\s*(.+)/)?.[1]?.trim() || "Imported Project";
  const repoUrl = content.match(/- 项目地址:\s*(.+)/)?.[1]?.trim() || "";
  const language =
    content.match(/- 编程语言:\s*(.+)/)?.[1]?.trim() || "Unknown";
  ...
  const filesBlock = content.match(
    /##\s*3\.\s*项目所有文件列表[\s\S]*?```text\s*([\s\S]*?)```/i,
  );
  const allFiles =
    filesBlock?.[1]
      ?.split("\n")
  ...
  const graphNodes = Array.isArray(callChain?.graph?.nodes)
    ? callChain.graph.nodes
    : [];
  const graphEdges = Array.isArray(callChain?.graph?.edges)
    ? callChain.graph.edges
    : [];
  ...
  let nodes = graphNodes.map((n: any) => {
    ...
    return {
      ...
      description:
        importedDescription ||
        (record?.status
          ? `导入节点（状态: ${record.status}，层级: ${record.depth ?? "-"})`
          : "导入节点"),
      module: "",
      depth: typeof record?.depth === "number" ? record.depth : undefined,
      drillFlag:
        typeof record?.drillFlag === "number" ? record.drillFlag : undefined,
      callStatus:
  ...
  if (visualization.modules.length > 0) {
    ...
    nodes = nodes.map((node) => ({
      ...node,
      module: moduleIdSet.has(visualization.nodeModuleMap[node.id])
        ? visualization.nodeModuleMap[node.id]
        : node.module,
    }));
    modules = restored;
  }
...
function parseVisualizationDataFromPanoramaMarkdown(content: string): {
  ...
  const sectionRegex = new RegExp(
    `${VISUALIZATION_SECTION_TITLE}[\\s\\S]*?\\\`\\\`\\\`json\\s*([\\s\\S]*?)\\\`\\\`\\\``,
    "i",
  );
  ...
  try {
    ...
    const nodeModuleMap =
      parsed?.nodeModuleMap && typeof parsed.nodeModuleMap === "object"
        ? Object.fromEntries(
            Object.entries(parsed.nodeModuleMap).map(([k, v]) => [
              String(k),
              String(v || ""),
            ]),
          )
    ...
    const nodeDescriptions =
      parsed?.nodeDescriptions && typeof parsed.nodeDescriptions === "object"
        ? Object.fromEntries(
            Object.entries(parsed.nodeDescriptions).map(([k, v]) => [
              String(k),
              String(v || ""),
            ]),
          )
...
function parseAgentLogsFromPanoramaMarkdown(content: string): LogEntry[] {
  const sectionRegex = new RegExp(
    `${AGENT_LOG_SECTION_TITLE}[\\s\\S]*?\\\`\\\`\\\`json\\s*([\\s\\S]*?)\\\`\\\`\\\``,
    "i",
  );
  const match =
    content.match(sectionRegex) ||
    content.match(/"agentLogs"\s*:\s*(\[[\s\S]*\])/i);
  if (!match?.[1]) return [];
  try {
    const parsed = JSON.parse(match[1]);
    ...
    return parsed
      .map((item: any) => ({
        ...
        type: (["info", "success", "error", "thinking"].includes(item?.type)
          ? item.type
          : "info") as LogEntry["type"],
        details: Array.isArray(item?.details)
          ? item.details.map(String)
          : undefined,
        aiTrace:
          item?.aiTrace && typeof item.aiTrace === "object"
            ? {
                request: String(item.aiTrace.request || ""),
                response: String(item.aiTrace.response || ""),
                label: item.aiTrace.label
                  ? String(item.aiTrace.label)
                  : undefined,
              }
            : undefined,
      }))
...
function buildExportMarkdownWithAgentLogs(
  ...
  const stripped = baseMarkdown
    .replace(new RegExp(`${AGENT_LOG_SECTION_TITLE}[\\s\\S]*$`, "i"), "")
    .trimEnd();
...
function parseAiUsageStatsFromPanoramaMarkdown(content: string): AiUsageStats {
  const sectionRegex = new RegExp(
    `${AI_USAGE_SECTION_TITLE}[\\s\\S]*?\\\`\\\`\\\`json\\s*([\\s\\S]*?)\\\`\\\`\\\``,
    "i",
  );
  const match = content.match(sectionRegex);
  if (!match?.[1]) {
...
function App() {
  ...
  const [settings, setSettings] = useState<AppSettings>(() => {
    ...
    try {
      ...
      return {
        ...
        maxDrillDepth: Math.max(
          1,
          Math.min(5, Number(parsed?.maxDrillDepth || 2) || 2),
        ),
        maxChildCallsPerFunction: Math.max(
          5,
          Math.min(20, Number(parsed?.maxChildCallsPerFunction || 10) || 10),
        ),
      };
    } catch {
      return {
  ...
  const [homeSourceType, setHomeSourceType] = useState<"github" | "local">(
    "github",
  );
  ...
  const [isProjectFilesFullscreenOpen, setIsProjectFilesFullscreenOpen] =
    useState(false);
  ...
  const [importedAiUsageStats, setImportedAiUsage] =
    useState<AiUsageStats | null>(null);
  ...
  }>({ armed: false, lastSavedFingerprint: "" });
  ...
  const mergedAiUsageStats = useMemo<AiUsageStats>(
    () => ({
      inputTokens:
        (importedAiUsageStats?.inputTokens || 0) +
        (aiUsageStats.inputTokens || 0),
      outputTokens:
        (importedAiUsageStats?.outputTokens || 0) +
        (aiUsageStats.outputTokens || 0),
      callCount:
        (importedAiUsageStats?.callCount || 0) + (aiUsageStats.callCount || 0),
    }),
    [importedAiUsageStats, aiUsageStats],
  );
  const shouldShowReanalyzeModulesButton =
    moduleClassificationFailed ||
    ((graphData?.nodes?.length || 0) > 0 &&
      (graphData?.modules?.length || 0) === 0);
  ...
  const { enabled: devDiagnosticsEnabled, append: appendDevDiagnostic } =
  ...
  const normalizedPanelWidths = useMemo(() => {
    const total = visiblePanelKeys.reduce(
      (acc, key) => acc + Math.max(1, panelWidths[key]),
      0,
    );
  ...
  useEffect(() => {
    ...
    const hydrateLlmDefaultsFromEnv = async () => {
      try {
        const response = await fetch("/api/settings/llm-defaults", {
          cache: "no-store",
        });
  ...
  const handleAnalyze = (
    ...
    analyzeRepo(
      url,
      sourceType === "github" ? githubToken : undefined,
      sourceType,
    );
  };
  ...
  const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        let parsedAiUsage: AiUsageStats = {
          inputTokens: 0,
          outputTokens: 0,
          callCount: 0,
        };
        ...
        if (
          !importedData &&
          /PROJECT_PANORAMA|函数调用链（JSON）|##\s*4\./i.test(content)
        ) {
        ...
        if (importedData && importedData.nodes && importedData.edges) {
              ...
              return {
                ...
                drillFlag:
                  typeof node.drillFlag === "number"
                    ? node.drillFlag
                    : typeof record.drillFlag === "number"
                      ? record.drillFlag
                      : undefined,
                callStatus:
                  typeof node.callStatus === "string"
                    ? node.callStatus
          ...
          if (
            importedData.aiUsageStats &&
            typeof importedData.aiUsageStats === "object"
          ) {
            parsedAiUsage = {
              inputTokens:
                Number(importedData.aiUsageStats.inputTokens || 0) || 0,
              outputTokens:
                Number(importedData.aiUsageStats.outputTokens || 0) || 0,
          ...
          hydrateImportedContext(
            ...
            importedData.panoramaMarkdown ||
              ...
              content.includes("PROJECT_PANORAMA")
                ? content
                : ""),
          );
          setView("result");
          ...
          if (importedData) {
            ...
            alert(
              "无法识别的文件格式。请导入工程文件（PROJECT_PANORAMA_*.md）或兼容 JSON 文件。",
            );
          }
        }
      } catch (error: any) {
  ...
  const handleLoadHistory = useCallback(
    async (id: string) => {
      ...
      try {
        const item = readLocalHistoryRecords().find(
          (record) => record.id === id,
        );
        if (!item?.graphData) {
        ...
        hydrateImportedContext(
          item.graphData,
          String(item.markdown || item.graphData?.panoramaMarkdown || ""),
        );
        setView("result");
      } catch (error: any) {
  ...
  const handleDeleteHistory = useCallback(
    async (id: string) => {
      ...
      try {
        const nextItems = readLocalHistoryRecords().filter(
          (record) => record.id !== id,
        );
  ...
  useEffect(() => {
    ...
    const sourceType: "github" | "local" = /^https?:\/\//i.test(source)
      ? "github"
      : "local";
    const name =
      graphData.repoName ||
    ...
    const run = async () => {
      try {
        const nextRecord: HistoryStoredRecord = {
          ...
          techStack: Array.isArray(graphData.project?.techStack)
            ? graphData.project.techStack.slice(0, 20)
            : [],
          mdFile: "",
        ...
        const existing = readLocalHistoryRecords().filter(
  ...
  const openFileInSource = useCallback(
    async (filePath: string, line?: unknown) => {
      if (!filePath) return;
      const lineNumber = toLineNumber(line);
        ...
        ? [
            {
              ...
              options: {
                ...
                className:
                  theme === "dark"
                    ? "source-target-line-dark"
                    : "source-target-line-light",
                linesDecorationsClassName:
                  theme === "dark"
                    ? "source-target-gutter-dark"
                    : "source-target-gutter-light",
              },
            },
          ]
        : [],
  ...
  const handleSourceEditorMount = useCallback<OnMount>(
    (editor, monaco) => {
    ...
    >
      <div
        className={clsx(
          ...
          theme === "dark"
            ? "border-stone-600 bg-stone-800 shadow-black/60 ring-1 ring-stone-700/50"
            : "border-stone-200 bg-white shadow-stone-400/30",
        )}
      ...
      >
        ...
        <div
          className={clsx(
            "flex-1 overflow-y-auto p-6 space-y-6 text-sm scrollbar-custom",
        ...
        >
          <section>
            ...
            >
              ...
              <div className="mt-1">
                本页面中的全部设置仅保存在当前浏览器本地，包括外观、GitHub
                配置、AI
                模型配置和分析参数；这些数据不会保存到服务端，也不会同步给其他设备或用户。
              </div>
              <a
          ...
          <section>
            ...
            <div
              className={clsx(
                "rounded-xl border p-4 space-y-6",
                theme === "dark"
                  ? "border-stone-700 bg-stone-900/50"
                  : "border-stone-200 bg-stone-50/50",
              )}
            >
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex flex-col">
                    ...
                    <div
                      className={clsx(
                        "text-[10px] mt-0.5 max-w-[200px] leading-relaxed",
                        theme === "dark" ? "text-stone-500" : "text-stone-400",
                      )}
                    >
                      控制 AI 沿着函数调用链向下探索的层数。深度越大，分析越透彻，但耗时更久。
                    </div>
                  </div>
                  <div className="flex flex-col items-end">
                    <span
                      className={clsx(
                        "tabular-nums text-sm font-bold h-6 flex items-center px-2 rounded-md",
                        theme === "dark" ? "bg-amber-500/10 text-amber-400" : "bg-amber-50 text-amber-600",
                      )}
                    >
                      {settings.maxDrillDepth} 层
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span
                    className={clsx(
                      "text-[10px] tabular-nums font-mono opacity-50",
                      theme === "dark" ? "text-stone-500" : "text-stone-400",
                    )}
                  >
                    1
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSettings((prev) => ({
                        ...prev,
                        maxDrillDepth: Number(e.target.value),
                      }))
                    }
                    className={clsx(
                      "flex-1 h-1.5 rounded-full appearance-none cursor-pointer transition-all",
                      "[&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:shadow-lg [&::-webkit-slider-thumb]:cursor-pointer [&::-webkit-slider-thumb]:hover:scale-110 [&::-webkit-slider-thumb]:transition-transform",
                      theme === "dark"
              ...
              <div
                className={clsx(
                  ...
                  theme === "dark" ? "border-stone-700/60" : "border-stone-200",
                )}
              />
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex flex-col">
                    ...
                    >
                      每个函数内最多提取多少个子调用。数值越大，图表越丰满，过大可能导致视觉杂乱。
                    </div>
                  </div>
                  <div className="flex flex-col items-end">
                    <span
                    ...
                    >
                      {settings.maxChildCallsPerFunction} 个
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSettings((prev) => ({
                        ...prev,
                        maxChildCallsPerFunction: Number(e.target.value),
                      }))
                    }
                    className={clsx(
                      "flex-1 h-1.5 rounded-full appearance-none cursor-pointer transition-all",
        ...
        <div className="w-full max-w-5xl flex flex-col items-center gap-8 relative z-10">
          <div className="text-center space-y-6">
            ...
            <div>
              <h1 className="text-5xl font-bold tracking-tight mb-3">
                <span
                  className={theme === "dark" ? "text-white" : "text-stone-900"}
                >
                  代码
                </span>{" "}
          ...
          <div className="w-full relative">
            <div className="max-w-2xl mx-auto">
              {homeSourceType === "github" ? (
                <RepoInput
                  onAnalyze={(url) => handleAnalyze(url, "github")}
                  isLoading={isAnalyzing}
                  theme={theme}
                  topContent={homeSourceTabs}
                />
              ) : (
                <LocalPathInput
                  onAnalyze={(path) => handleAnalyze(path, "local")}
                  isLoading={isAnalyzing}
                  theme={theme}
                  topContent={homeSourceTabs}
                />
            ...
            <div className="mt-10">
              ...
              {historyError ? (
                <div
                  className={clsx(
                    ...
                    theme === "dark"
                      ? "border-rose-700/40 text-rose-300 bg-rose-500/10"
              ...
              {historyLoading ? (
                <div
                  className={clsx(
                    "rounded-lg border px-3 py-6 text-center text-xs",
                    theme === "dark"
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {pagedHistoryItems.map((item) => {
                      ...
                      const langColorMap: Record<
                        string,
                        { dark: string; light: string }
                      > = {
                        TypeScript: {
                          dark: "text-blue-300 bg-blue-500/15",
                          light: "text-blue-700 bg-blue-50",
                        },
                        JavaScript: {
                          dark: "text-yellow-300 bg-yellow-500/15",
                          light: "text-yellow-700 bg-yellow-50",
                        },
                        Python: {
                          dark: "text-sky-300 bg-sky-500/15",
                          light: "text-sky-700 bg-sky-50",
                        },
                        Go: {
                          dark: "text-cyan-300 bg-cyan-500/15",
                          light: "text-cyan-700 bg-cyan-50",
                        },
                        Rust: {
                          dark: "text-orange-300 bg-orange-500/15",
                          light: "text-orange-700 bg-orange-50",
                        },
                        Java: {
                          dark: "text-red-300 bg-red-500/15",
                          light: "text-red-700 bg-red-50",
                        },
                        "C++": {
                          dark: "text-pink-300 bg-pink-500/15",
                          light: "text-pink-700 bg-pink-50",
                        },
                        C: {
                          dark: "text-indigo-300 bg-indigo-500/15",
                          light: "text-indigo-700 bg-indigo-50",
                        },
                        Ruby: {
                          dark: "text-rose-300 bg-rose-500/15",
                          light: "text-rose-700 bg-rose-50",
                        },
                        PHP: {
                          dark: "text-violet-300 bg-violet-500/15",
                          light: "text-violet-700 bg-violet-50",
                        },
                        Swift: {
                          dark: "text-orange-300 bg-orange-500/15",
                          light: "text-orange-700 bg-orange-50",
                        ...
                        Kotlin: {
                          dark: "text-purple-300 bg-purple-500/15",
                          light: "text-purple-700 bg-purple-50",
                        },
                        Vue: {
                          dark: "text-emerald-300 bg-emerald-500/15",
                          light: "text-emerald-700 bg-emerald-50",
                        },
                      };
                      const langKey = item.language || "";
                      const langColors = langColorMap[langKey] || {
                        dark: "text-stone-300 bg-stone-700/50",
                        light: "text-stone-600 bg-stone-100",
                      };
                      return (
                        <div
                          key={item.id}
                        ...
                        >
                          <button
                            type="button"
                            onClick={() => {
                              void handleLoadHistory(item.id);
                            }}
                            disabled={Boolean(
                              loadingHistoryId || deletingHistoryId,
                            )}
                ...
                >
                  <div
                    className={clsx(
                      ...
                      theme === "dark"
                        ? "text-stone-400 border-stone-800 bg-stone-900/60"
                  ...
                  <div
                    className={clsx(
                      ...
                      theme === "dark"
                        ? "border-stone-800 bg-stone-900/50 text-stone-300"
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <div className="font-mono truncate">
                        {mergedAiUsageStats.inputTokens.toLocaleString()}
                      </div>
                    </div>
                    <div className="min-w-0">
                      <div
                      ...
                      <div className="font-mono truncate">
                        {mergedAiUsageStats.outputTokens.toLocaleString()}
                      </div>
                    </div>
                    <div className="min-w-0">
                      <div
                      ...
                      <div className="font-mono truncate">
                        {mergedAiUsageStats.callCount.toLocaleString()}
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (!target) return;
                        const sourceType = /^https?:\/\//i.test(target)
                          ? ("github" as const)
                          : ("local" as const);
                ...
                <div className="space-y-2.5">
                  {(graphData?.modules || []).map((mod) => (
                    <button
                      ...
                      style={{
                        borderColor:
                          activeModule === mod.id ? mod.color : undefined,
                      }}
                    >
                      <div
                ...
                >
                  ...
                  <pre
                    className={clsx(
                      ...
                      theme === "dark"
                        ? "text-stone-300 scrollbar-custom"
                        : "text-stone-700 scrollbar-custom",
                    )}
                  >
        ...
        >
          ...
          >
            <div className="flex items-center gap-4 min-w-0">
              <div
                className={clsx(
                  "text-sm font-medium flex items-center gap-2 shrink-0",
                  theme === "dark" ? "text-stone-400" : "text-stone-500",
                )}
              >
                <FolderTree size={16} />
          ...
          <div className="flex-1 relative overflow-hidden">
            {visiblePanelKeys.length === 0 ? (
              ...
              <Group
                ...
                defaultLayout={visiblePanelKeys.reduce<Record<string, number>>(
                  (acc, key) => {
                  ...
                  <React.Fragment key={panelKey}>
                    ...
                    >
                      {panelKey === "files" && (
                        <section
                          className={clsx(
                            "h-full min-w-[140px] flex flex-col",
                            theme === "dark" ? "bg-stone-900/70" : "bg-white",
                          )}
                        >
                          <div
                            className={clsx(
                              "px-4 py-3 border-b text-sm font-medium shrink-0 flex items-center gap-2",
                              theme === "dark"
                          ...
                          <div
                            className={clsx(
                              "flex-1 min-h-0 overflow-auto p-2 scrollbar-custom",
                              theme === "dark"
                                ? "text-stone-300"
                                : "text-stone-700",
                            )}
                      ...
                      {panelKey === "source" && (
                        <section
                          className={clsx(
                            "h-full min-w-[240px] flex flex-col",
                            theme === "dark" ? "bg-stone-900" : "bg-white",
                          )}
                        >
                          <div
                            className={clsx(
                              "px-4 py-3 border-b text-sm font-medium shrink-0 flex items-center justify-between",
                              theme === "dark"
                                ? "border-stone-800 text-stone-300 bg-stone-900/60"
                                : "border-stone-200 text-stone-700 bg-stone-50",
                            )}
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                className={clsx(
                                  "text-xs font-mono truncate max-w-[360px]",
                                  theme === "dark"
                                    ? "text-stone-400"
                                    : "text-stone-500",
                                )}
                                title={selectedFile || "未选择文件"}
                              >
                          ...
                          <div
                            className={clsx(
                              "px-3 py-1.5 border-b text-[11px] font-mono",
                              theme === "dark"
                          ...
                          <div
                            className={clsx(
                              "flex-1 min-h-0 overflow-auto scrollbar-custom",
                              theme === "dark" ? "bg-stone-900" : "bg-white",
                            )}
                          >
                            {sourceLoading ? (
                              ...
                              <Editor
                                ...
                                language={detectMonacoLanguageFromPath(
                                  selectedFile,
                                )}
                                value={sourceCode}
        ...
        >
          <div
            className={clsx(
              ...
              theme === "dark"
                ? "border-stone-600 bg-stone-800 shadow-black/60 ring-1 ring-stone-700/50"
                : "border-stone-200 bg-white/95 shadow-stone-400/30",
            )}
          >
            <div
              className={clsx(
          ...
          >
            ...
            <div
              className={clsx(
                ...
                theme === "dark"
                  ? "bg-stone-800 text-stone-200 scrollbar-custom"
                  : "bg-white text-stone-700 scrollbar-custom",
              )}
            >
